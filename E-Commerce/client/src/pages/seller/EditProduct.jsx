...
const EditProduct = () => {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', category: '', subCategory: '', brand: '', status: 'active', isNewArrival: false, isTrending: false, shortDescription: '', slug: '', tags: '', features: '', returnPolicy: 'No Returns'
  });
  ...
  useEffect(() => {
    ...
    if (product && product._id === id) {
      ...
      if (mappedVariants.length === 0) {
        mappedVariants.push({
          colorName: '', swatchPreview: '', swatchFile: null, originalPrice: 0, price: 0, stock: 0, sku: '', sizes: [],
                          ...
                          <div className="space-y-2">
                            {variant.sizes.map((s, sIndex) => (
                              <div key={sIndex} className="flex items-center gap-2">
                                <select value={s.name} onChange={(e) => updateSize(vIndex, sIndex, 'name', e.target.value)} className="flex-1 bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-black">
